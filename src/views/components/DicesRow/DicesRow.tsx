...
const DicesRow = (props: Props) => {
    ...
    const dicesAnimationElement = <img src={dicesAnimation} style={{ padding: '0 2px', width: '60px' }} alt="Dices" />;

    if (props.attacking) {
        attackerDices = props.dices.attacker.map(() => {
            return dicesAnimationElement;
        });

        defenderDices = props.dices.defender.map(() => {
            return dicesAnimationElement;
        });
    } else if (props.dices) {
        attackerDices = props.dices.attacker.map((dice: number) => {
            return (
                <img src={dicesSrc[dice - 1]} style={{ padding: '0 2px', width: '60px' }} alt="Dices" />
            );
        });
