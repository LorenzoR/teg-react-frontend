...
import GameHeader from '../Header/Header';
...
export interface Props {
  ...
  initGame: (payload: { countries: {}[], players: Player[] }) => void;
...
class LayoutWrapper extends Component<Props> {
        ...
        if (messageData.body) {
          ...
          switch (action) {
            ...
            case 'gameStarted': {
              ...
              this.props.setGameStatus({ gameStatus });
              this.props.initGame({ countries, players });
  ...
  finishRound = () => {
    ...
    const player = _.find(this.props.players, (obj) => obj.id === this.state.currentPlayerId);
  ...
  render() {
    ...
    const source = _.find(this.props.countries, (obj) => obj.countryKey === this.state.countrySelection.source);
    ...
    const modals = (
      <div>
        <SelectPlayerColorModal
          ...
          isAdmin={this.props.currentPlayer && this.props.currentPlayer.isAdmin}
        ...
        <CountryCardsModal
          ...
          countryCardsModalHandler={this.toggleCountryCardsModalVisibility}
    ...
    if (this.props.countries && currentPlayer) {
      _.forIn(this.props.countries, (value, key) => {
        if (currentPlayer && value.state.player.color === currentPlayer.color) {
          playerCountries += 1;
        }
      });
    }
